  ...
  <AppLayout ref="report__wrap">
    ...
    <van-collapse accordion v-model="activeName" class="mtop10">
      ...
      <van-collapse-item title="收费模板详情" name="2">
        <div class="content__item" v-for="item in routeData.chargeFeeTemplate && routeData.chargeFeeTemplate.once" :key="item.type">
          <span>型号{{ item.type }}:</span>
          <span>{{ item.price }}</span>
        </div>
        <div class="content__item" v-for="item in routeData.chargeFeeTemplate && routeData.chargeFeeTemplate.time" :key="item.type">
